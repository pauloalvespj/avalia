...
// Componente de select de setor local (sem afetar contexto global)
function SetorSelectLocal({ empresaAtiva, setorId, onChange }) {
...
export default function EscutaPage() {
  ...
  if (!setorId) return (
    <div>
      <SetorSelectLocal empresaAtiva={empresaAtiva} setorId={setorId} onChange={id => { setSetorId(id); setForm(FORM_VAZIO) }} />
      <div className="card text-center py-10 text-muted text-sm">Selecione um setor acima para registrar a escuta.</div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <PageHeader
        ...
        subtitle={setorId ? '' : 'Selecione um setor acima'}
